...
class RLTrainer(Trainer):
    def __init__(self, *args, **kwargs):
        ...
        self.old_model = ref_model.eval()
        self.old_model.to('cuda:0')
        for param_name in parameter_names:
            param = self.old_model.get_parameter(param_name)
            param.requires_grad = False
        
        self.clip_epsilon = kwargs.pop('clip_epsilon', 0.2)
        self.vacab_size = self.model.pretrained_model.config.vocab_size
        self._step_count = 0
        self._loss_count = 0
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        with torch.no_grad():
            old_logits, _, _ = self.old_model(**inputs, output_hidden_states=True, return_dict=True)
        ...
        if not torch.all(shift_labels==IGNORED_INDEX):
            shift_mask = mask[..., :-1].contiguous()
            shift_old_logits = old_logits[..., :-1, :].contiguous()
            log_probs = F.log_softmax(shift_logits, dim=-1)
            old_log_probs = F.log_softmax(shift_old_logits, dim=-1)
            kl = log_probs.exp() * (log_probs - old_log_probs)
            kl = kl * shift_labels.ne(IGNORED_INDEX).unsqueeze(-1) * ~shift_mask.unsqueeze(-1)
            kl_loss = kl.sum() / (shift_labels.ne(IGNORED_INDEX).sum() + 1e-6)
            all_losses = kl_loss+value_weight*value_loss
        else:
            all_losses = value_weight*value_loss
