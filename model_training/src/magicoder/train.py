...
def get_gpu_memory_usage():
    result = subprocess.run(['gpustat'], capture_output=True, text=True)
    output = result.stdout

    gpu_info = []
    for line in output.split('\n')[1:]: 
        match = re.search(r'\[(\d+)\].*?(\d+) / \d+ MB', line)
        if match:
...
def set_cuda_visible_devices(gpu_id):
    ...
    print(f"已将CUDA_VISIBLE_DEVICES设置为{gpu_id}")
...
def wait():
    ...
    while True:
        ...
        for gpu_id, memory_used in gpu_info:
            if gpu_id==0 and memory_used < 4000:
