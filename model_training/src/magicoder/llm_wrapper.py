...
from typing import Callable, Literal, Any, Dict, List, Optional, Tuple, Union
import os
import torch
...
class TokenizationContext:
    ...
    @staticmethod
    def from_model_key(
        model_key, model_name_or_path: str | None = None
    ) -> "TokenizationContext":
        ...
        model_key = model_name_or_path if model_name_or_path is not None else model_key
        tokenizer = AutoTokenizer.from_pretrained(model_key, use_fast=use_fast)
...
def get_model_context(
    ...
    elif model_key in SupportedModelKeys.qwencoder_based_models():
        max_context_size = 16384
    else:
        assert model_key in SupportedModelKeys.deepseekcoder_based_models() 
        max_context_size = 16384
    if tokenization_context is None:
        tokenization_context = TokenizationContext.from_model_key(model_name_or_path)
...
def load_valuehead_params(path_or_repo_id, model_args):
    r"""
    ...
    try:
        vhead_file = os.path.join(path_or_repo_id, V_HEAD_WEIGHTS_NAME) #cached_file(filename=V_HEAD_WEIGHTS_NAME, **kwargs)
...
def create_infilling_prompt(
    ...
    if model_key in SupportedModelKeys.starcoder_based_models():
        ...
        assert tokenizer is not None
        return form_qwencoder_infill(tokenizer, prefix, suffix)
