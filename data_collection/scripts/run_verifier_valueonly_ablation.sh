...
RUN_NAME="verifier_valueonly_ablation_20260420"
...
BASE_RAW="${ROOT}/model_training/review_mcts_train_data/verifier_valueonly_baseline_raw_20260420.jsonl"
VALUE_RAW="${ROOT}/model_training/review_mcts_train_data/verifier_valueonly_augmented_raw_20260420.jsonl"
...
BASE_OUT="${ROOT}/model_training/src/output/review-lora-verifier-valueonly-baseline-160step"
VALUE_OUT="${ROOT}/model_training/src/output/review-lora-verifier-valueonly-augmented-160step"
...
convert_and_filter() {
  ...
  if [[ ! -f "${VALUE_RAW}" ]]; then
    echo "[stage:${CURRENT_STAGE}] value-only correction raw start=$(date -Is)"
    PYTHONDONTWRITEBYTECODE=1 \
    PYTHONPATH="${ROOT}/model_training/src" \
    HF_HUB_OFFLINE=1 \
    UV_CACHE_DIR=/tmp/uv-cache \
    uv run python -m magicoder.preprocess_review_mcts_data \
      --input "${INPUTS[@]}" \
      --output_file "${VALUE_RAW}" \
      --policy_min_q 0.5 \
      --max_value_paths_per_dimension 0 \
      --emit_verifier_corrections \
      --verifier_correction_mode value_only \
      --verifier_correction_repeat 1 \
      --verifier_correction_q 0.65 \
      --verifier_correction_lm_weight 0.0 \
      --verifier_correction_value_weight 0.5 \
      --shuffle_seed 20260420
  else
    echo "[stage:${CURRENT_STAGE}] value-only raw exists: ${VALUE_RAW}"
  fi
  ...
  if [[ ! -f "${VALUE_DATA}" ]]; then
    echo "[stage:${CURRENT_STAGE}] value-only filter start=$(date -Is)"
    PYTHONDONTWRITEBYTECODE=1 \
    PYTHONPATH="${ROOT}/model_training/src" \
    TRL_EXPERIMENTAL_SILENCE=1 \
    HF_HUB_OFFLINE=1 \
    UV_CACHE_DIR=/tmp/uv-cache \
    uv run python -m magicoder.filter_review_train_data \
      --input_file "${VALUE_RAW}" \
      --output_file "${VALUE_DATA}" \
      --model_key Qwen/Qwen3.5-9B \
      --model_name_or_path "${MODEL_PATH}" \
      --max_tokens 1152 \
      --min_policy_items 10 \
    ...
    echo "[stage:${CURRENT_STAGE}] value-only filtered exists: ${VALUE_DATA}"
  fi
  wc -l "${BASE_RAW}" "${VALUE_RAW}" "${BASE_DATA}" "${VALUE_DATA}"
}
...
wait_for_train() {
  ...
  pid_base=$!
  train_one "valueonly" 1 "${VALUE_DATA}" "${VALUE_OUT}"
  pid_value=$!
...
wait_for_eval() {
  ...
  pid_base=$!
  eval_one "valueonly" 1 "${VALUE_OUT}"
  pid_value=$!
  echo "[stage:${CURRENT_STAGE}] waiting baseline pid=${pid_base}, valueonly pid=${pid_value}"
  wait "${pid_base}"
  wait "${pid_value}"
...
root = Path("/data1/xianzhiwei/mcts-code-review/model_training/src/output/review-eval-verifier_valueonly_ablation_20260420")
rows = {}
for name in ["baseline", "valueonly"]:
    ...
    if path.exists():
        ...
        rows[name] = {
            ...
            "grade_median_abs_error": summary.get("grade_median_abs_error"),
            "boundary_acc": summary.get("boundary_acc"),
            "low_grade_false_positive_rate": summary.get("low_grade_false_positive_rate"),
            "high_grade_false_negative_rate": summary.get("high_grade_false_negative_rate"),
            "unsupported_evidence_rate": summary.get("unsupported_evidence_rate"),
        }
