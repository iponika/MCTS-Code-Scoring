...
- Replaced the initial `vllm 0.10.2` attempt after confirming it did not support `Qwen3_5ForConditionalGeneration`; installed `vllm 0.19.1` instead, which resolves the Qwen3.5 architecture and binds to `torch 2.10.0+cu128` on the server's NVIDIA 570.133.20 / CUDA 12.8 driver stack.
- Removed the cloned environment's stale `flash-attn 2.7.4.post1` binary after it failed against `torch 2.10.0` with an undefined `c10::cuda::SetDevice` symbol; vLLM 0.19.1 then loaded Qwen3.5 through its own supported path.
- Verified the isolated environment imports `torch 2.10.0+cu128`, `transformers 5.8.0`, and `vllm 0.19.1`, and ran a focused project smoke (`test_train_multi_qwen_messages.py`, `test_direct_bootstrap_review.py`) successfully.
- Ran a Qwen3.5-9B vLLM smoke from a real script file (`scripts/qwen35_vllm_smoke.py`) after avoiding stdin-based multiprocessing: vLLM resolved `Qwen3_5ForConditionalGeneration`, loaded all 4 safetensor shards, generated a short response, and exited with GPU resources released.
- Added Qwen3.5-9B direct step-count AXIOM experiment support: no-think and thinking configs, `scripts/qwen35_thinking_mode_probe.py`, and `data_collection/scripts/run_direct_stepcount_axiom_qwen35_9b.sh`. The new entry sources the isolated `/data1/ruiqi/conda_envs/mcts-vllm-qwen35` environment, uses plain `python`/`accelerate` instead of system `uv`, checks GPU idleness before GPU stages, defaults no-think to `Static`, `Direct Review`, `1/2/3-step`, and defaults thinking to `Direct Review` plus `1-step`.
- Added explicit evaluation-time Qwen chat-template thinking control (`--chat_template_enable_thinking`) so no-think and thinking training are evaluated with the same template mode instead of relying on tokenizer defaults.
- Probed Qwen3.5-9B no-think behavior. Tokenizer rendering with `enable_thinking=False` differs from default/think by producing an empty `<think></think>` prefill, and a short vLLM probe confirmed `enable_thinking=False` generates a direct answer without `Thinking Process`; a literal user `/no_think` suffix alone did not suppress thinking unless the chat-template flag is also set false.
- Cloned the missing datasets into the project-local `/data1` workspace: `datasets/axiom-llm-judge` and `datasets/CodeCriticBench`. A balanced AXIOM seed smoke confirmed grades 1-5 are available from `datasets/axiom-llm-judge/axiombench`.
- Added `disable_custom_all_reduce` support to local vLLM config/init and enabled it for Qwen3.5-9B configs. The first TP=2 vLLM attempt failed in `custom_all_reduce.cuh`; with custom all-reduce disabled, a 5-sample no-think TP=2 `STOP_AFTER_DATA=1` smoke generated direct review plus 1/2/3-step raw data and preprocessing outputs successfully.
- Raised Qwen3.5 experiment truncation ceilings to reduce truncation confounds: data generation prompt problem/code character limits default to 24000, AXIOM seed filtering defaults to 24000 code characters, training-data preprocessing now exposes `--max_problem_chars/--max_code_chars`, Qwen3.5 preprocessing passes 24000, evaluation passes the same 24000 limits, and Qwen3.5 training sequence length defaults to 4096 tokens.
...
- 2026-05-04: Changed only the final-review prior-thought placement contract in `shared/prompt_contract.py`: when `force_final=True` and prior reasoning exists, the builder now appends that reasoning directly under instruction context with `You don't have to analyze code by yourself.`, while the final `@@ Response` side remains reserved for the `<review>` prefill. The step prompt path still keeps prior reasoning in `@@ Response` / assistant history.
- 2026-05-04: Refined that same contract further: the final-turn synthesis sentence and `REVIEW_FINAL_CONSISTENCY_RULE` are now injected only when prior reasoning actually exists, and they appear immediately before `You don't have to analyze code by yourself.`. Also made the step prompt default response prefix start with `1. ` when no earlier reasoning is present, so explicit numbered-step experiments do not start from an empty assistant continuation.
- 2026-05-04: Updated the prompt-visibility and prompt-alignment tests to match the current numbered-step experiment contract: step prompts are now checked for `1. ` response-prefix guidance and prior-thought placement semantics rather than the older "Generate one concise native reasoning note" wording, while final-prompt tests now distinguish the with-prior vs 0-step cases.
- 2026-05-04: Diagnosed the current `Qwen3-4B no_think` stepwise failure mode more precisely: most invalid stepwise finals were not pure truncation, but `missing_open_only` outputs where the model emitted a closing `</review>` without the opening `<review>` block being preserved in saved text.
- 2026-05-04: Fixed anchored-final reconstruction in `model_training/src/magicoder/review_evaluator.py`. Chat-template / anchored final prompts already inject `FINAL_REVIEW_PREFILL`, but generation returns only the continuation, so the evaluator now restores the anchored prefix before parsing/storing final reviews and retries. `parse_final_review()` also gained a conservative recovery path for closing-only JSON suffixes such as `2, "functional_correctness": false ... </review>`.
- 2026-05-04: Raised the default final-review evaluation budget again from `768` to `1536` tokens in `model_training/src/magicoder/batch_review_evaluator.py`, `model_training/src/magicoder/review_evaluator.py`, and `data_collection/scripts/run_axiom_clean_eval.sh`, so remaining static/final-only failures are less likely to be simple token exhaustion.
- 2026-05-04: Raised `data_collection/configs/mcts_code_review_qwen3_4b_no_think.yaml:max_tokens` from `384` to `1536` to keep direct-bootstrap data generation aligned with the looser final-review budget now that the 4B pipeline has ample VRAM headroom.
- 2026-05-04: Fixed the same anchored-final prefix bug on the data-generation side in `data_collection/direct_bootstrap_review.py`. Freeform/anchored final normalization now uses `merge_final_review_text()` instead of blindly prepending `FINAL_REVIEW_PREFILL`, which avoids malformed outputs like `<review>{"axiom_grade": {"axiom_grade": 0, ...}` when the model already begins its continuation with a full JSON object.
- 2026-05-04: Added targeted regression coverage in `tests/test_review_evaluator_parsing.py` and `tests/test_direct_bootstrap_review.py` for both anchored-final recovery cases: restoring the prefix for closing-only suffixes and avoiding double `axiom_grade` prefixes when the continuation already starts with `{`.
- 2026-05-04: Re-ran a 1-sample `Qwen3-4B no_think` 2-step smoke after the fixes. The model now reliably emits numbered visible step text (`1.` in the first node, later continuation reached `4.` because the first node itself emitted `1./2./3.` inside one segment), and the final generated review no longer has the duplicated `{"axiom_grade": {"axiom_grade": ...}}` corruption.

## 2026-05-05

- Changed the stepwise final-turn prompt contract for prompts with prior reasoning. When earlier numbered analysis notes exist, `shared/prompt_contract.py` now moves all prior reasoning into instruction context, explicitly tells the model not to continue the numbered notes, requires the very first output token to be `<review>`, and leaves the final `@@ Response` section empty instead of anchoring it as an assistant-prefix continuation. This isolates whether the current stepwise failure mode comes from continuation semantics rather than scoring content.
- Updated `tests/test_review_prompt_visibility.py` and `tests/test_prompt_contract_alignment.py` to lock the new contract: stepwise final prompts with prior reasoning must include the "Do not continue the numbered analysis notes" and "Start your very first output token with <review>" instructions, while 0-step final prompts must omit those hints and keep the anchored final prefill path.
- Generalized `data_collection/scripts/run_direct_stepcount_vs_review_qwen3_4b.sh` so the training phase now handles any number of `STEP_COUNTS` values instead of only the first two. Extra stepwise variants are trained in additional rounds (parallel in pairs, then single on the remaining GPU), which is required for overnight `1,2,3-step` no-think comparison runs.
- Added `data_collection/prepare_axiom_seedset.py`, a balanced AXIOM-only seed builder that emits the repo's normalized `prepared_review_sample` format for grades 1-5 and preserves original AXIOM subset/row provenance in `dataset_index`, `source_dataset`, and `original_dataset_index`.
- Added `data_collection/scripts/run_direct_stepcount_axiom_qwen3_4b.sh`, a no-think AXIOM wrapper around the existing direct-vs-stepcount pipeline. It prebuilds an AXIOM-only seed set in the run directory, defaults to `STEP_COUNTS=1,2,3`, and then reuses the standard training/eval pipeline so overnight same-distribution AXIOM comparisons do not mix in CodeCriticBench mapping noise.
- Fixed a late-stage orchestration bug in `data_collection/scripts/run_direct_stepcount_vs_review_qwen3_4b.sh`: `write_summary()` now runs its embedded Python with `PYTHONPATH="${ROOT}"`, so same-distribution overnight runs no longer fail after all generation, training, and evaluation have already finished just because the final `summary.json` step cannot import `data_collection.review_experiment_utils`.
- Marked `data_collection/scripts/run_direct_stepcount_axiom_qwen3_4b.sh` executable so the AXIOM overnight wrapper can be launched directly from tmux without relying on the caller to set the mode bit first.
