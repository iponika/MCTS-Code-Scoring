...
目前的目标是让模型对代码质量给出更稳定、更可信的分数；评论文本主要作为分数的证据和可解释性补充。

当前阶段已经完成从数据生成、训练到 value-guided 推理的最小闭环。

现阶段最明确的正向结果是在 CodeCritic 同分布保留测试集上，trained direct 相对 base direct 有提升，使用 response_mean_value 的 value-guided MCTS 在 3 次重复中进一步把 MAE 从 1.450 降到 1.089，boundary accuracy 从 0.775 提升到 0.804，unsupported evidence 从 0.100 降到 0.018。

但在 AXIOM 保留测试集上结果是负面的，trained value-guided MCTS 反而更差，主要问题是格式稳定性下降和对正确代码过度低估。这说明当前方法还没有证明通用评分能力，只证明了同分布训练和 value-guided 推理有潜力。接下来需要着重提升泛化能力，并设计更公平、更可信的评测。
...
### 4.4 评分语义清洗与重平衡后的跨数据集小规模实验

针对 4.3 和随后一次短提示训练中的负结果，我们做了一次修正实验。问题背景是：模型训练不只是把“答案文本”喂给模型，还会让模型学习训练样本里的表达风格。上一版静态评分数据中有较模板化的证据句，例如“监督来源可靠”“该标签来自某数据集”，这类文本对人来说只是元信息，但对模型来说可能被学成评分理由，导致推理时给出空泛证据。同时，长样本被 1152 token 长度限制过滤后，保留下来的训练样本中 grade=1 明显偏多，模型容易学成“保守低分”。

这次实验的设计是：

| 设计项 | 具体做法 |
|---|---|
| 评分标准 | 仍使用 AXIOM 0-5，其中 0-2 表示功能不正确，3-5 表示功能正确 |
| 模型 | Qwen3.5-9B + LoRA + value head |
| 训练方式 | policy 学习 `<review>` JSON 输出；value head 学习 AXIOM value |
| 数据清洗 | 去掉训练响应中的数据集来源、标签来源、监督可靠性等元信息 |
| 数据重平衡 | 先按真实训练 prompt 计算 token 长度并过滤，再在可训练样本中按 AXIOM 0-5 档位抽样 |
| 推理对照 | Direct 表示模型直接给一个评分；value-rerank 表示模型生成两个候选评分，由 value head 选择更可信的候选 |

训练数据来自 AXIOM、CodeCritic、Code-DiTing、CodeJudgeBench。先构造 7962 条候选样本，其中 4702 条能放入 1152 token 训练长度。最后抽取 824 条训练样本：
...
其中精确或近似精确评分样本按分数档位基本均衡：

| AXIOM grade | 0 | 1 | 2 | 3 | 4 | 5 |
|---:|---:|---:|---:|---:|---:|---:|
| 样本数 | 120 | 120 | 120 | 104 | 120 | 120 |

训练 600 step 后，在固定的 100 条跨数据集评测集上测试：AXIOM 30 条、CodeCritic 30 条、Code-DiTing 20 条、CodeJudgeBench 20 条。结果如下：

| 方法 | valid_rate | MAE | boundary_acc | interval_acc | pairwise_acc |
|---|---:|---:|---:|---:|---:|
| Base Direct | 0.950 | 1.268 | 0.625 | 0.537 | 0.111 |
| 短提示训练 Direct | 1.000 | 1.517 | 0.600 | 0.430 | 0.100 |
| Clean-balanced Direct | 1.000 | 1.233 | 0.667 | 0.420 | 0.300 |
| Clean-balanced Value-rerank | 1.000 | 1.133 | 0.700 | 0.440 | 0.300 |

这个结果说明，简单缩短 prompt 不一定有效，甚至会让 MAE 变差；但“清洗训练文本 + 先过滤后重平衡分数档位”是有效的。clean-balanced value-rerank 相比基础模型直接评分，MAE 从 1.268 降到 1.133，功能正确性边界准确率从 0.625 提高到 0.700，输出格式有效率从 0.950 提高到 1.000。

不过这还不是最终结论。首先，评测集只有 100 条，规模较小；其次，interval_acc 仍低于 base direct，说明模型对弱区间标签和中间分数档的校准仍不稳定；第三，个别极端样本中 value head 仍会把错误候选选成高分候选。因此目前可以汇报为：清洗和重平衡能明显缓解低分偏置，并让 value-guided 推理首次在跨数据集小样本上取得正向趋势，但还需要更大样本、多 seed 重复和更严格的候选证据质量控制。
...
代码质量评分应该优化 MAE、boundary accuracy，还是应该更重视 high-FN/low-FP 的非对称风险？
...
3. 是否要实现pairwise ranking？它可能比绝对分数更稳健，但需要额外训练实现。
4. 最终报告是否应该强调 value-guided MCTS 的推理收益，还是强调 MCTS 生成数据 + value head 训练的完整路线？
